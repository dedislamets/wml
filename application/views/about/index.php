<style type="text/css">
	@media (max-width: 575px){
		.about-low-area {
		     display: block; 
		     padding: 20px;
		}
	}
</style>
<div class="slider-area ">
	<div class="single-slider hero-overly slider-height2 d-flex align-items-center" data-background="assets/img/hero/about.jpg">
		<div class="container">
			<div class="row">
				<div class="col-xl-12">
					<div class="hero-cap">
						<h2>About us</h2>
						<nav aria-label="breadcrumb">
							<ol class="breadcrumb">
								<li class="breadcrumb-item"><a href="<?= base_url(); ?>">Home</a></li>
								<li class="breadcrumb-item"><a href="#">About</a></li>
							</ol>
						</nav>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>

<div class="about-low-area section-padding30">
	<div class="container">
		<div class="row">
			<div class="col-md-12 mb-35">
				<div id="carouselExampleControls" class="carousel slide" data-ride="carousel">
					<ol class="carousel-indicators">
					    <li data-target="#carouselExampleIndicators" data-slide-to="0" class="active"></li>
					    <li data-target="#carouselExampleIndicators" data-slide-to="1"></li>
					</ol>
				  	<div class="carousel-inner">
					    <div class="carousel-item active">
					      <img class="d-block w-100" src="<?= base_url(); ?>assets/img/slide1.jpeg" >
					    </div>
					    <div class="carousel-item">
					      <img class="d-block w-100" src="<?= base_url(); ?>assets/img/slide2.jpeg">
					    </div>
			
				  	</div>
					<a class="carousel-control-prev" href="#carouselExampleControls" role="button" data-slide="prev">
					    <span class="carousel-control-prev-icon" aria-hidden="true"></span>
					    <span class="sr-only">Previous</span>
					</a>
					<a class="carousel-control-next" href="#carouselExampleControls" role="button" data-slide="next">
					    <span class="carousel-control-next-icon" aria-hidden="true"></span>
					    <span class="sr-only">Next</span>
					</a>
				</div>
			</div>
			<div class="col-lg-6 col-md-12">
				<div class="about-caption mb-50">

					<div class="section-tittle mb-35">
						<span>Vision and Mission Company</span>
						<h2>Vision</h2>
					</div>
					<p>To become a strong logistics company which set a customer safety and satisfaction as our priority</p>
					<div class="section-tittle mb-35">
						<h2>Mission</h2>
					</div>
					<ul>
						<li style="list-style: square;">Providing a various logistics and multi-modal transportation services</li>
						<li style="list-style: square;">Integrated logistics services improvement that bring benefit for the community</li>
						<li style="list-style: square;">Providing the excellent service with the competitive prices.</li>
						<li style="list-style: square;">Establish the network connection in various regions both in Indonesia and international</li>
						<li style="list-style: square;">Facilitating the development of Human Resources So that it is valuable, professional, and highly competent</li>
						<li style="list-style: square;">Provide the warehousing services for various types of goods with the best handling</li>
						<li style="list-style: square;">Continuously develop service in the field of logistics</li>
					</ul>
				</div>
			</div>
			<div class="col-lg-6 col-md-12">

				<div class="about-img wow slideInRight" data-wow-delay="400ms" data-wow-duration="1500ms">
					<div class="about-font-img">
						<img class="img img-responsive" src="<?= base_url(); ?>assets/img/Pengiriman-Sampel-COVID-19-wahana-multi-logistik-2-1024x768.jpeg" alt="">
					</div>
				</div>
			</div>
		</div>
	</div>
</div>